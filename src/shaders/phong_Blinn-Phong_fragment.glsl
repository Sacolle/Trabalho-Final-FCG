...
void main()
{
    ...
    if(dot(normalize(p - light_pos),light_dir) < cos(light_angle))
        iluminado_spotlight  = false;
    if(dot(normalize(p - flashlight_pos),flashlight_dir) < cos(flashlight_angle) || length(p - flashlight_pos) > flashlight_range)
