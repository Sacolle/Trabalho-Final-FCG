...
//coordenadas do ponto c da camera
glm::vec4 g_camera_c = glm::vec4(-1.0f,1.5f,-4.0f,1.0f);
//bools para mover a camera
bool g_key_w = false;
bool g_key_s = false;
bool g_key_d = false;
bool g_key_a = false;
...
int main()
{
    ...
    glfwSetScrollCallback(window, ScrollCallback); //usar a rodinha do mouse
    ...
    // Utilizaremos estas variáveis para enviar dados para a GPU
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		// pinta os pixels do framebuffer com a cor e reseta o z-buffer
		...
		// pos em coordenadas esféricas definidas pelo user input 
		...
		*/
        ...
        glm::mat4 view = Matrix_Camera_View(g_camera_c, camera_view_vector, camera_up_vector);
        ...
        for (int i = 1; i <= 3; ++i){
            ...
            glm::mat4 model;
			if (i == 1){
				//matriz modelo do cubo 1 eh a matriz identidade, pois ele não sobre alterações
                ...
                model = Matrix_Translate(0.0f, 0.0f, -2.0f) // TERCEIRO translação
                      ...
                      * Matrix_Scale(2.0f, 0.5f, 0.5f); // PRIMEIRO escala (cisalha)
            }
            else if ( i == 3 ){
                //bubo 3 rota em X, Y e Z (nessa ordem) seguindo o sistema de ângulos de Euler, e após uma translação em X.
            ...
            // Enviamos a matriz "model" para a GPU.
            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));

            // Informamos para a placa de vídeo (GPU) que a variável booleana
            // "render_as_black" deve ser colocada como "false".
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if(key == GLFW_KEY_W && action == GLFW_PRESS) g_key_w = true;
	if(key == GLFW_KEY_W && action == GLFW_RELEASE) g_key_w = false;
    if(key == GLFW_KEY_S && action == GLFW_PRESS) g_key_s = true;
	if(key == GLFW_KEY_S && action == GLFW_RELEASE) g_key_s = false;
    if(key == GLFW_KEY_D && action == GLFW_PRESS) g_key_d = true;
	if(key == GLFW_KEY_D && action == GLFW_RELEASE) g_key_d = false;
    if(key == GLFW_KEY_A && action == GLFW_PRESS) g_key_a = true;
	if(key == GLFW_KEY_A && action == GLFW_RELEASE) g_key_a = false;
}
